...
root.title("Donate plasma, save a life :)")
...
def save():
    ...
    s = '\n' + now.strftime(
        "%d-%m-%Y %H:%M") + '\t' + e1.get() + '\t' + e2.get() + '\t ' + e3.get() + '\t' + d + '\t' + gender + ' \t' + course
    f = open(('donor.txt'), 'a')
    f.write(s)
    f.close()

    # save data in csv file
    with open('donor.csv', 'a') as fs:
        w = csv.writer(fs)
        w.writerow(
...
l1 = Label(root, text="Donor Registration form", width=25, font=("times", 20, "bold"), bg='blue',
           fg='white')
l1.place(x=70, y=50)
...
l3 = Label(root, text="Address", width=20, font=("times", 12, "bold"), anchor="w", bg='grey')
l3.place(x=70, y=180)
e2 = Entry(root, width=30, bd=2)
e2.place(x=240, y=180)
l4 = Label(root, text="Date of testing negative", width=20, font=("times", 12, "bold"), anchor="w", bg='grey')
l4.place(x=70, y=230)

# dateEntry -Date selection entry with drop-down calendar
dob = DateEntry(root, width=27, background='brown', foreground='white', date_pattern='dd/mm/Y',
                borderwidth=3)
dob.place(x=240, y=230)

l6 = Label(root, text="Date of testing negative", width=20, font=("times", 12, "bold"), anchor="w",
           bg='grey')
l6.place(x=70, y=230)
